...
import (
	...
	omni "github.com/voxgig-sdk/hano-snapchat-lens-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HanoSnapchatLensError); is {
				out := map[string]any{
					"name":    "HanoSnapchatLensError",
...
func omniCtx(arg any, client *sdk.HanoSnapchatLensSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HanoSnapchatLensSDK); is {
			client = live
