...
func TestLensEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Lens(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Lens(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		lensRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.lens")))
...
func lensBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HANO_SNAPCHAT_LENS_TEST_LENS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HANO_SNAPCHAT_LENS_TEST_LENS_ENTID"])
