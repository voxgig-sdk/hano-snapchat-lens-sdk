# hano-snapchat-lens-cli

boru-driven command-line client **and** interactive REPL for the HanoSnapchatLens
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hano-snapchat-lens-cli)
make build

# 2. See usage (words, entities, env vars)
./hano-snapchat-lens-cli --help

# 3. Provide credentials once, via the environment
export HANO_SNAPCHAT_LENS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hano-snapchat-lens-cli list lens

# 5. Override the API base URL for a single call
HANO_SNAPCHAT_LENS_BASE=https://api.example.com ./hano-snapchat-lens-cli list lens

# 6. No arguments -> interactive REPL
./hano-snapchat-lens-cli
hano-snapchat-lens> list lens
hano-snapchat-lens> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hano-snapchat-lens-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hano-snapchat-lens-cli list lens
   ```
...
Run with no arguments to open a REPL (prompt `hano-snapchat-lens>`). Each line is
evaluated as its own boru expression:

```text
$ ./hano-snapchat-lens-cli
hano-snapchat-lens> list lens
hano-snapchat-lens> /help
hano-snapchat-lens> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `lens`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hano-snapchat-lens-cli`. |
...
(`./hano-snapchat-lens-cli <expr>`) and interactively (the REPL), and expressions compose the
