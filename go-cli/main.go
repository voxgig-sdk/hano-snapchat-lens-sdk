// Command HanoSnapchatLens-cli is a CLI + REPL for the HanoSnapchatLens SDK,
...
const prompt = "hano-snapchat-lens"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "lens"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HANO_SNAPCHAT_LENS_APIKEY (api key), HANO_SNAPCHAT_LENS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HANO_SNAPCHAT_LENS_APIKEY carries the API key and
	// HANO_SNAPCHAT_LENS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HANO_SNAPCHAT_LENS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HANO_SNAPCHAT_LENS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHanoSnapchatLensSDK(opts)
