...
//     (\`new HanoSnapchatLensSDK(...)\` / \`HanoSnapchatLensSDK.test(...)\` become
//     \`HanoSnapchatLensSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HanoSnapchatLensSDK'
...
const TEST_SEED = {"entity":{"lens":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HanoSnapchatLensSDK(...)\` and any
// \`HanoSnapchatLensSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HanoSnapchatLensSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HanoSnapchatLensSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
