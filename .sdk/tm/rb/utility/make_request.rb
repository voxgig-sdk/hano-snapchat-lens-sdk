...
module HanoSnapchatLensUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HanoSnapchatLensResponse.new({})
    ...
    if fetch_err
      ...
      response = HanoSnapchatLensResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HanoSnapchatLensResponse.new(fetched)
    else
