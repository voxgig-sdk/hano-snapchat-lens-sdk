...
//    only - a typed HanoSnapchatLensContext there would make every ctx
...
final class HanoSnapchatLensOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HanoSnapchatLensContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HanoSnapchatLensSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HanoSnapchatLensResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HanoSnapchatLensError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HanoSnapchatLensResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HanoSnapchatLensError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HanoSnapchatLensError(is_string($code) ? $code : '', $msg);
    }
