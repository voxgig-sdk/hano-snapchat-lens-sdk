...
// HanoSnapchatLens SDK retry feature
...
class HanoSnapchatLensRetryFeature extends HanoSnapchatLensBaseFeature
{
    ...
    public function init(HanoSnapchatLensContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HanoSnapchatLensContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HanoSnapchatLensContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HanoSnapchatLensContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
