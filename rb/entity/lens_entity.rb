...
class LensEntity
  ...
  def make
    opts = @_entopts.dup
    LensEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HanoSnapchatLensHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Lens, Hash] the current Lens data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HanoSnapchatLensHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Lens fields)
  def match_get
  ...
  #
  # @param reqmatch [LensListMatch, Hash, nil] match filter (any subset of
  #   Lens fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Lens>, Array] the matching Lens items; raises HanoSnapchatLensError on failure
