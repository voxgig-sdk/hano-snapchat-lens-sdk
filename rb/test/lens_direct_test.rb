...
class LensDirectTest < Minitest::Test
  def test_direct_list_lens
    setup = lens_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-lens", setup[:live] ? "live" : "unit")
    if _should_skip
...
def lens_direct_setup(mockres)
  ...
  if live
    ...
    client = HanoSnapchatLensSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
