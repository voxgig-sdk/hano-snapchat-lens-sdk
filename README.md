...
The Hano Lens by حنين العزاوي allows users to apply a unique and artistic filter to their selfies, enhancing facial features and adding a touch of creativity.

Learn more about Hano Snapchat Lens at [lens.snapchat.com](https://lens.snapchat.com).
...
$lenss = $client->Lens()->list();
print_r(array_map(fn($item) => $item->data_get(), $lenss));
```
