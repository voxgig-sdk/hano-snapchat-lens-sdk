...
[MCP](https://modelcontextprotocol.io) server exposing the HanoSnapchatLens SDK as
two agent tools — `hano-snapchat-lens_list` and `hano-snapchat-lens_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hano-snapchat-lens-mcp)
make build

# 2. Provide credentials via the environment
export HANO_SNAPCHAT_LENS_APIKEY=sk_live_xxx
...
```jsonc
// hano-snapchat-lens_list: first page of records
{ "entity": "lens" }
{ "entity": "lens", "query": { } }

// hano-snapchat-lens_load: one record by id
{ "entity": "lens", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hano-snapchat-lens-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hano-snapchat-lens \
     -- "$PWD"/dist/*/hano-snapchat-lens-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hano-snapchat-lens_list` and `hano-snapchat-lens_load` tools now appear
   in new sessions. Ask the agent to *"list lens using hano-snapchat-lens"*
   and it calls `hano-snapchat-lens_list` with `{"entity":"lens"}`.
...
```sh
export HANO_SNAPCHAT_LENS_APIKEY=sk_live_xxx            # API key
export HANO_SNAPCHAT_LENS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hano-snapchat-lens_list` tool
...
### Call the `hano-snapchat-lens_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hano-snapchat-lens_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hano-snapchat-lens_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HANO_SNAPCHAT_LENS_APIKEY` | API key sent with every request. |
| `HANO_SNAPCHAT_LENS_BASE` | Optional override of the API base URL. |
