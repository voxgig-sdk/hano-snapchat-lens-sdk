...
// HanoSnapchatLens SDK feature test
//
...
class FtCtrl extends HanoSnapchatLensControl
{
...
class FtClient
{
    ...
    private HanoSnapchatLensUtility $utility;

    public function __construct(HanoSnapchatLensUtility $utility, array $options)
    {
    ...
    public function get_utility(): HanoSnapchatLensUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HanoSnapchatLensContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HanoSnapchatLensContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HanoSnapchatLensContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HanoSnapchatLensConfig::make_feature($name);
    ...
    private static function buildUrl(HanoSnapchatLensSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HanoSnapchatLensError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HanoSnapchatLensResponse($fetched);
        }
    ...
    private function fail(HanoSnapchatLensContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HanoSnapchatLensContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
