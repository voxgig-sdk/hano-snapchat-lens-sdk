...
class LensDirectTest extends TestCase
{
    public function test_direct_list_lens(): void
    {
        $setup = lens_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-lens", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function lens_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new HanoSnapchatLensSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
