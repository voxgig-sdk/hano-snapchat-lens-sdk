...
// HanoSnapchatLens SDK Lens entity
...
class LensEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LensEntity($this->_client, $opts);
    }

    /**
     * @param Lens|array $args Lens data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HanoSnapchatLensHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Lens|array The current Lens data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Lens fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HanoSnapchatLensHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Lens fields).
     */
    ...
    /**
     * List Lens items matching the given filter.
     *
     * @param LensListMatch|array|null $reqmatch Match filter (any subset
     *   of Lens fields) as an assoc-array; LensListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Lens[]|array A list of Lens items as assoc-arrays at
     *   the SDK boundary; throws HanoSnapchatLensError on failure (item-5 convention).
     */
