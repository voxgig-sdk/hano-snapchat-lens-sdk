...
// HanoSnapchatLens SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class HanoSnapchatLensMakePoint
{
    public static function call(HanoSnapchatLensContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof HanoSnapchatLensError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = HanoSnapchatLensHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = HanoSnapchatLensHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
