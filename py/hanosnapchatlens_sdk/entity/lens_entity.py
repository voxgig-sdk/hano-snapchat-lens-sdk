...
from hanosnapchatlens_sdk.hanosnapchatlens_types import (
    Lens,
    LensListMatch,
)
...
class LensEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LensEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Lens]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Lens().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
