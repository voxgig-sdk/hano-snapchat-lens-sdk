# HanoSnapchatLens SDK test runner
...
class HanoSnapchatLensTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HanoSnapchatLensTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HanoSnapchatLensTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HanoSnapchatLensTestRunner.getenv("HANO_SNAPCHAT_LENS_TEST_LIVE")
        override = HanoSnapchatLensTestRunner.getenv("HANO_SNAPCHAT_LENS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HanoSnapchatLensTestRunner.getenv(key)
        ...
        explain = HanoSnapchatLensTestRunner.getenv("HANO_SNAPCHAT_LENS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HANO_SNAPCHAT_LENS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HanoSnapchatLensTestRunner._test_control is not None:
            return HanoSnapchatLensTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HanoSnapchatLensTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HanoSnapchatLensTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HanoSnapchatLensTestRunner.load_test_control()
...
def load_env_local():
    HanoSnapchatLensTestRunner.load_env_local()


def env_override(m):
    return HanoSnapchatLensTestRunner.env_override(m)


def entity_data(v):
    return HanoSnapchatLensTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HanoSnapchatLensTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HanoSnapchatLensTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HanoSnapchatLensTestRunner.load_test_control()


def live_client_options():
    return HanoSnapchatLensTestRunner.live_client_options()


def live_delay_ms():
    return HanoSnapchatLensTestRunner.live_delay_ms()
