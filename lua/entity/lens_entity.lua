...
local LensEntity = {}
LensEntity.__index = LensEntity


function LensEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LensEntity)
  self._name = "lens"
  self._client = client
...
function LensEntity:get_name()
  return self._name
end


function LensEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LensEntity.new(self._client, opts)
end
...
function LensEntity:mark_deleted()
  self._deleted = true
end


function LensEntity:deleted()
  return true == self._deleted
end


function LensEntity:data_set(args)
  if args ~= nil then
...
function LensEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LensEntity:match_set(args)
  if args ~= nil then
...
function LensEntity:stream(action, args, callopts)
...
function LensEntity:_run_op(ctx, post_done)
  local utility = self._utility
